...
SELECT E.EMPLOYEE_ID
 ...
 INNER JOIN EMPLOYEES E_MAN
    ON E.MANAGER_ID = E_MAN.EMPLOYEE_ID
 WHERE E.HIRE_DATE < E_MAN.HIRE_DATE
 ORDER BY E.EMPLOYEE_ID ASC
;
...
SELECT E.EMPLOYEE_ID
     ...
     , E.DEPARTMENT_ID
  FROM EMPLOYEES E
 INNER JOIN (SELECT AVG(SALARY) AS AVG_SALARY
 				  , DEPARTMENT_ID
 			   FROM EMPLOYEES
...
SELECT FIRST_NAME_LENGTH
 ...
 ORDER BY FIRST_NAME_LENGTH ASC
;
...
;
-- 124-2. 국가 아이디의 첫 글자와 국가명의 첫 글자가 같은 
--           국가의 이름, 국가 아이디, 해당 국가의 부서 이름을 조회한다.
SELECT C.COUNTRY_ID
	 , C.COUNTRY_NAME
	 , D.DEPARTMENT_NAME
  FROM (SELECT COUNTRY_ID
		  ...
		  FROM (SELECT COUNTRY_ID
					 ...
					 , SUBSTR(COUNTRY_NAME, 1, 1) AS NAME_FIRST_LETTER
				  FROM COUNTRIES)
		 WHERE ID_FIRST_LETTER = NAME_FIRST_LETTER) C
 INNER JOIN LOCATIONS L
 	ON L.COUNTRY_ID = C.COUNTRY_ID
 INNER JOIN DEPARTMENTS D
    ON L.LOCATION_ID = D.LOCATION_ID
...
SELECT FIRST_NAME_LETTER
 ...
 ORDER BY FIRST_NAME_LETTER ASC
;
...
SELECT RANK
  ...
  FROM (SELECT HIRE_MONTH
			 ...
			 , ROW_NUMBER() OVER( ORDER BY MAX_SALARY DESC ) AS RANK
		  FROM (SELECT HIRE_MONTH
...
SELECT RANK
	 ...
	 , MAX_SALARY
  FROM (SELECT HIRE_MONTH
			 , ROW_NUMBER() OVER(ORDER BY HIRE_MONTH ASC) AS RANK
			 , MAX(SALARY) AS MAX_SALARY
		  FROM (SELECT TO_CHAR(HIRE_DATE, 'MM') AS HIRE_MONTH
				     , SALARY
				  FROM EMPLOYEES)
...
SELECT DEPARTMENT_NAME
 ...
 WHERE DEPARTMENT_ID IN (SELECT DEPARTMENT_ID
						   FROM (SELECT DEPARTMENT_ID
									  , RANK() OVER( ORDER BY GAP DESC ) AS RANK
								   FROM (SELECT DEPARTMENT_ID
...
-- 155. 도시별로 근무 주인 사원의 수를 조회한다. 근무중인 사원이 없는 도시는 0으로 조회한다.
SELECT L.CITY
...
SELECT L.CITY
  ...
  LEFT OUTER JOIN (SELECT D.LOCATION_ID
  					 ...
  					 FROM DEPARTMENTS D
  					INNER JOIN EMPLOYEES E
  					   ON D.DEPARTMENT_ID = E.DEPARTMENT_ID) D_E
...
SELECT E.EMPLOYEE_ID 
 ...
 INNER JOIN JOBS J 
    ON E.JOB_ID = J.JOB_ID
  LEFT OUTER JOIN (SELECT PAST_D.DEPARTMENT_NAME
  					    , PAST_J.JOB_TITLE
  					    , JH.EMPLOYEE_ID
  					 FROM JOB_HISTORY JH
